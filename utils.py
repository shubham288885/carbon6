...
def set_environment_variables():
    ...
    os.environ["NVIDIA_API_KEY"] = "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" #set API key
...
def describe_image(image_content):
    ...
    #new
    response = requests.post(invoke_url, headers=headers, json=payload).json()
    if "choices" not in response:
        logging.error(f"Invalid NVIDIA API response: {response}")
